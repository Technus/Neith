...
namespace NeithDevices.serial
{
    public class CSSSC:IDisposable
    {
            ...
            if (IsClosed)
            {
                ...
                finally
                {
                    ...
                    else
                    {
                        ...
                        else if (errorValue == (uint)NativeMethods.FileError.ERROR_FILE_NOT_FOUND)
                        {
                            throw new Exception("Failed to CreateFile Port not found");
                        }
                        else
                        {
                            throw new Exception("Failed to CreateFile Case unknown " + errorValue);
                        }
                    }
                }
            }
        ...
        private uint GetLastError()
        {
            try
            {
                return UnsafeNativeMethods.GetLastError();
            }
            catch (Exception e)
            {
                Debug.Print("Failed to read Error!");
                Debug.Print(e.ToString());
                return 0xFFFFFFFF;
            }
        }
    ...
    internal static class UnsafeNativeMethods
    {
        ...
        public static extern uint WaitForSingleObject(IntPtr eventHandle, uint millis);

        [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Unicode)]
        public static extern IntPtr CreateEventA(IntPtr lpSecurityAttributes, bool ManualReset, bool InitialState, IntPtr lpName);
        ...
        [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Unicode)]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static extern bool ReadFile(IntPtr hFile, [Out] IntPtr lpBuffer, uint nNumberOfBytesToRead,
        ...
        [DllImport("kernel32.dll", SetLastError = true)]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static extern bool GetCommProperties(IntPtr hFile, ref NativeMethods.CommProp lpCommProp);

        [DllImport("kernel32.dll", SetLastError = true)]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static extern bool GetCommModemStatus(IntPtr hFile,
            [MarshalAs(UnmanagedType.U4)] out NativeMethods.ModemStat lpModemStat);

        [DllImport("kernel32.dll", SetLastError = true)]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static extern bool GetCommMask(IntPtr hFile,
        ...
        [DllImport("kernel32.dll", SetLastError = true)]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static extern bool GetCommTimeouts(IntPtr hFile,
        ...
        [DllImport("kernel32.dll", SetLastError = true)]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static extern bool PurgeComm(IntPtr hFile, [MarshalAs(UnmanagedType.U4)] NativeMethods.PurgeFlags dwFlags);

        [DllImport("kernel32.dll", SetLastError = true)]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static extern bool GetCommState(IntPtr hFile, ref NativeMethods.DCB lpDCB);

        [DllImport("kernel32.dll", SetLastError = true)]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static extern bool SetCommState(IntPtr hFile, [In] ref NativeMethods.DCB lpDCB);
        ...
        public static extern NativeMethods.FileType GetFileType(IntPtr hFile);
        ...
        [DllImport("kernel32.dll", SetLastError = true)]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static extern bool EscapeCommFunction(IntPtr hFile,
